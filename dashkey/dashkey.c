...
#ifdef OPENCLACK
//openclack keymatrix
uint8_t keycodes[16][8] PROGMEM = {{0x29, 0x00, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F},	//pb row 0, keycode matrix; edit this to change keymap. Fn slotted in as 0xFF
							       {0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47},	//pf row 0
							       {0x35, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24},	//pb row 1
							       {0x25, 0x26, 0x27, 0x2D, 0x2E, 0x2A, 0x49, 0x4A},	//pf row 1
							       {0x2B, 0x14, 0x1A, 0x08, 0x15, 0x17, 0x1C, 0x18},	//pb row 2
							       {0x0C, 0x12, 0x13, 0x2F, 0x30, 0x31, 0x4C, 0x4D},	//pf row 2
							       {0x39, 0x04, 0x16, 0x07, 0x09, 0x0A, 0x0B, 0x0D},	//pb row 3
							       {0x0E, 0x0F, 0x33, 0x34, 0x00, 0x28, 0x4B, 0x48},	//pf row 3
							       {0xE1, 0x1D, 0x1B, 0x06, 0x19, 0x05, 0x11, 0x10},	//pb row 4
							       {0x36, 0x37, 0x38, 0x00, 0x00, 0xE5, 0x4E, 0x52},	//pf row 4
							       {0xE0, 0xE3, 0xE2, 0x00, 0x00, 0x2C, 0x00, 0x00},	//pb row 5
							       {0x00, 0xE6, 0xE7, 0x65, 0xE4, 0x4F, 0x50, 0x51}};	//pf row 5
#else
//dashkey-custom keymatrix
...
ISR(TIMER0_COMPA_vect)
{
	...
	if(tempkey == 5) { //We're on the last active row, so we add an extra shift amount to our bit
		...
		PORTC |= 0x9F;
		PORTC &= (~(1<<tempkey))|0xE0; //We keep the last 3 pins high to prevent a duplicate of ROW5, and to prevent messing with the indicator LEDs
	}
#endif
